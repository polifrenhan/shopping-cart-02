...
from repository.user_repository import find_user_by_email, update_user
...
async def update_address_by_id_on_bd(
    user_email: str, address_id: str, updated_address: dict
):
    user = await find_user_by_email(user_email)
    address_list = user["addresses"]
    for address in address_list:
